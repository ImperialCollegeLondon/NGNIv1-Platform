function [clu, tree,error] = run_cluster(handles)
dim=handles.par.inputs;
fname=regexprep(handles.par.fname,'\W','');
fname_in=regexprep(handles.par.fname_in,'\W','');
...
try
    ...
    disp('no spikes or clusters found for this electrode');
    error =1;
 % If you get errors saying something is too long in param.c the issue is
 % filename length
end
