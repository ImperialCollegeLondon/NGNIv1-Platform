...
const char cfg_suffix[] = "_asic_cfg.bin";
...
bool open_sig_files(const char* path, long rec_time, iprobe_sig_t* obj)
{
	...
	obj->config.lsb_size = lsb_size_array[4];
	...
	if(rec_time == -1)
	{
		/* Look for all time indices */
		apr_finfo_t finfo;
		long indices[N_FILES_MAX];
		const char* types[N_FILES_MAX];
		size_t n_files_found = 0;
		...
		while(apr_dir_read(&finfo, APR_FINFO_NAME, input_dir) == APR_SUCCESS)
		{
			...
			for(i=0; i<N_SUPPORTED_TYPES;i++)
			{
				if(strcmp(finfo.name+strlen(finfo.name)-strlen(supported_types[i]),
					supported_types[i]) == 0)
				{
					types[n_files_found] = supported_types[i];
					valid_suffix = true;
					break;
				}
			}
			...
			if(valid_suffix)
			{
				if(sscanf(finfo.name, "Rec_%d",&indices[n_files_found]) == 1)
				{
					/* Check that this is file 000 */
					char file_seq[4];
					memcpy(file_seq, finfo.name+11,3);
					file_seq[3] = '\0';
					if(strcmp(file_seq, "000") == 0)
						n_files_found++;
				}
			}
		}
		...
		if(n_files_found == 1)
		{
			printf("Found index: %6d (%s).\n", indices[0], types[0]);
			rec_time = indices[0];
		}
		else
		{
			printf("Found %d suitable records.\n", n_files_found);
			int i=0;
			for(;i<n_files_found;i++)
			{
				printf("[%d] %06d (%s)\n", i+1, indices[i], types[i]);
			}
			
			/* Obtain the selection from the user */
			printf("Selection: ");
			int sel = -1;
			while(sel<1 || sel>n_files_found)
			{
				scanf("%d", &sel);
			}
			rec_time = indices[sel-1];
		}
	}
...
bool dump_sig_to_mat(iprobe_sig_t* obj, const char* folder, size_t read_buf_size, size_t write_buf_size, uint64_t copy_limit)
{
	...
	char default_folder[MAX_PATH_LEN];
	if(!folder)
	{
		sprintf(default_folder, "%s/mat", obj->path);
		folder = (const char*) default_folder;
	}
	...
	if(copy_limit)
	{
		printf("Only first %llu seconds will be copied\n", copy_limit);
		
		/* Convert copy_limit to records */
		copy_limit *= N_CHANNELS_MAX;
		copy_limit *= obj->config.sampling_rate;
	}
	...
	for(i_file=0; i_file<obj->n_files; i_file++)
	{
		...
		/* Check if we only want to read a part of the file */
		if(copy_limit)
		{
			uint64_t data_available = cur_size + total_copied;
			if(data_available > copy_limit*sizeof(uint16_t))
			{
				cur_size = copy_limit*sizeof(uint16_t)-total_copied;
			}
		}
		
		printf("Processing %llu B\n", cur_size);
